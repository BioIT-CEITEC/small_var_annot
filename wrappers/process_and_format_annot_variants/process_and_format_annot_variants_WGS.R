...
run_all <- function(args){
  ...
  annot_tab <- fread(annot_file,showProgress = T)
  ...
  # write filtered tables for WGS
  write_out_per_chromosome(final_formated_tab,per_sample_results_dir,full_format_configs,output_file,col_config)
  ...
  ##WGS upravit
  cores <- 50
  chunks <- parallel::splitIndices(nrow(final_formated_tab), ncl = cores)
  cast_var_tab <- data.table()
  
  for(i in 1:cores){
  final_formated_tab_TEMP <- as.data.table(final_formated_tab[chunks[[i]],])
    
  var_sample_presence <- data.table::dcast.data.table(final_formated_tab_TEMP,formula = var_name ~ sample,fun.aggregate = identity,fill = 0,value.var = "is_in")
  final_formated_tab_TEMP[,is_in := NULL]
  unique_var_tab <- unique(final_formated_tab_TEMP,by = c("var_name","Gene_symbol"))
  unique_var_tab[,sample := NULL]
  cast_var_tab_TEMP <- merge(var_sample_presence,unique_var_tab,by = "var_name")
  setcolorder(cast_var_tab_TEMP,c("var_name","Gene_symbol"))
  
  cast_var_tab <- rbindlist(list(cast_var_tab, cast_var_tab_TEMP), fill=TRUE)
  message(sprintf("%i out of %s iterations DONE", i, cores ))
  rm(cast_var_tab_TEMP)
  suppressMessages(gc())
  }
  
  #
  fwrite(cast_var_tab,file = output_file,sep = "\t")
  openxlsx::write.xlsx(data.frame(),file = gsub(".tsv",".xlsx",output_file)) # WGS just empty, write tsv only
  ##############################################################################
...
filter_variants <- function(all_var_tab,VF_threshold = 0,coverage_alarm = c(1,5,10),single_transcript = T){
...
write_out_per_sample_vars  <- function(variant_tab,per_sample_results_dir,full_format_configs,output_file,col_config){
  ...
  for(my_sample in unique(variant_tab$sample)){
    ...
    if(!is.null(filtered_config)){
      ...
      openxlsx::write.xlsx(data.frame(),file = paste0(per_sample_results_dir,"/",my_sample,".variants.xlsx"))# WGS just empty, write tsv instead
      fwrite(c(res_list,res_list_add),file = paste0(per_sample_results_dir,"/",my_sample,".variants.tsv"),sep = "\t")#WGS new
      
      
    } else {
      openxlsx::write.xlsx(data.frame(),file = paste0(per_sample_results_dir,"/",my_sample,".variants.xlsx")) #WGS just empty, write tsv instead
      fwrite(sample_tab,file = paste0(per_sample_results_dir,"/",my_sample,".variants.tsv"),sep = "\t")#WGS new
    }
  }
  ## create empty files
  if(length(empty_sample_names) > 0){
    ...
    for(empty_sample_name in empty_sample_names){
      openxlsx::write.xlsx(data.frame(),file = paste0(per_sample_results_dir,"/",empty_sample_name,".variants.xlsx"))#just empty, write tsv instead
      #fwrite(sample_tab,file = paste0(per_sample_results_dir,"/",empty_sample_name,".variants.tsv"),sep = "\t")#new
...
write_out_per_chromosome  <- function(variant_tab,per_sample_results_dir,full_format_configs,output_file,col_config){
...
# develop and test
# setwd("/mnt/ssd/ssd_1/workspace/ROBIN2/MOII_signatures_GERMLINE/")
# args <- c("annotate/all_variants.annotated.processed.tsv",
#           "final_variant_table.tsv","per_sample_final_var_tabs",
#           "/mnt/references/general/germline_small_var_call_format_files/slaby_children_germ.txt","25",
#           "no_cohort_data","dont_save_cohort_data","variant_calling","/mnt/references/homo_sapiens/GRCh38-p10",
#           "homo_sapiens","mutation_loads.xlsx","germline_varcalls/MB0705krev.final_variants.tsv","germline_varcalls/FZ0711krev.final_variants.tsv")
